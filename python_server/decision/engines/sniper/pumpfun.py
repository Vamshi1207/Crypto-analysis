...
def snipe_veto(
    *,
    ...
    max_lift_pct: float = 120.0,
    dev_sold: bool = False,
    peak_real_sol: float = 0.0,
    curve_drop_pct: float = 50.0,
) -> Optional[str]:
    """Hard never-buy reasons. Ranking vs other creates lives in snipe_desk."""
    if create_px <= 0:
        return "no_create_px"
    ...
    if max_lift_pct > 0 and lift >= max_lift_pct:
        return "too_extended"
    return None
