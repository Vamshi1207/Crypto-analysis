...
import decision.engines.arb.amm as local_amm
...
def start(*, interval_s: Optional[float] = None) -> dict[str, Any]:
    ...
    with _lock:
        ...
        # Configure and start local AMM
        local_amm.configure_pools([
            {
                "pool_id": "EP2ib6dYdEeqD8MfE2ezHCxX3kP3K2eLKkirfPm5eyMx", # WIF pool
                "base_vault": "7e9ExBAvDvuJP3GE6eKL5aSMi4RfXv3LkQaiNZBPmffR",
                "quote_vault": "EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm",
                "base_decimals": 9, # SOL
                "quote_decimals": 6, # WIF
                "fee_pct": 0.25
            }
        ])
        local_amm.register_callback(_on_local_amm_price_change)
        local_amm.start()
...
def _on_local_amm_price_change(pool_id: str) -> None:
    # WIF mint
    mint = "EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm"
    # Trigger a scan for circular arbitrage when local pool updates
    opp = find_circular(mint)
    if opp:
        _scan_and_maybe_fill(target_mint=mint)
...
def _scan_and_maybe_fill(target_mint: Optional[str] = None) -> dict[str, Any]:
    universe = _universe_mints()
    if target_mint:
        universe = [m for m in universe if m[0] == target_mint]
